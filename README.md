...
- **비고 노출**: `비고:` 내용이 **장소** 또는 **제출/이행 여부**일 때만 아랫줄(라벨 없이 원문)로 표시
  - 제출/이행 키워드(`제출·접수·발송·납부·이행·체결·완료·미제출` 등) → 표시. 장소 키워드(`법원·검찰청·경찰서·법정·조정실` 등)나 전화번호 → 표시
  - 둘 다 아닌 비고(복대리·단순 메모 등)는 숨김. 키워드 동시 포함 시 제출이 우선
  - 장소성 비고는 기존 `장소` 줄과 **병기**(덮어쓰지 않음). 키워드는 `transform.py`의 `BIGO_*_KW` 표에서 조정
